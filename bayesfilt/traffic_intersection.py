...
class TrafficIntersection:
    """Class defining the traffic intersection"""
    ...
    def add_circular_zone(
        ...
        if radius is not None:
            self._df.loc[name, 'zone'] = Point(center).buffer(radius)
        else:
            self._df.loc[name, 'zone'] = Point(center)
        self._df.loc[name, 'type'] = ztype
        self._df.loc[name, 'orientation'] = orientation
    ...
    def add_arc_zone(
        ...
        pt_a, pt_b = list(left_border.coords)
        pt_c, pt_d = list(right_border.coords)
        spliter = LineString([pt_a, pt_b, pt_c, pt_d])
        circle = Point(center).buffer(radius)
        if reverse:
            self._df.loc[name, 'zone'] = split(circle, spliter)[0]
        else:
            self._df.loc[name, 'zone'] = split(circle, spliter)[1]
        self._df.loc[name, 'type'] = ztype
        self._df.loc[name, 'orientation'] = orientation
